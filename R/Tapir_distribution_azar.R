...
malla_wgs <- raster("./input/Malla_EcorII_wgs.tif")#wgs84
malla_lcc <- raster("./input/Malla_EcorII_lcc.tif")#LCC
plot(malla_lcc)
...
num <- 20
size <- rand_vect(N = num, M = freq(Tapir)[which(freq(Tapir)[,1] == 1),2], sd = 50, pos.only = TRUE, seed = 2)
size
...
result_random_S <- lapply(1:length(result_overlap_random) , function(x){
  setTxtProgressBar(pb, x)
  if(2 %in% unique(result_overlap_random[[x]][])){
    x<- result_overlap_random[[x]]; x[x != 2] <- NA  
  } else {
    x <- NULL
  }
  return(x)
...
plot(trim(result_random_S))

##No especies con intersect azar ----
base_area <- (res(result_overlap_random[[1]])[1]^2)* 1e-6#km2
pb <- txtProgressBar(0, length(result_overlap_random), style = 3)
Interseccion_M3 <- map_dfr(1:length(result_overlap_random), function(x){
  setTxtProgressBar(pb, x)
